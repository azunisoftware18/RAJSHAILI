...
const AnimatedVideoCard = ({ video, onClick, index }) => {
  ...
  if (embed?.type === "file") {
    ...
    previewContent = (
      <img
        src={getYouTubeThumbnail(embed.id)}
        alt={video.Name}
        className="w-full h-full object-cover"
      />
    );
    ...
    >
      ...
      <div className="p-5">
        ...
        <p className="text-sm text-slate-500 truncate">
          {video.profession || "No profession provided"}
        </p>
      </div>
    </div>
  );
...
export default function VideoManager() {
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      // Handle both object and array type responses
    ...
    if (ytMatch)
      return {
        ...
        url: `https://www.youtube.com/embed/${ytMatch[1]}${
          autoplay ? "?autoplay=1&controls=1&rel=0" : "?controls=1&rel=0"
        }`,
      };
    const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
    if (vimeoMatch)
  ...
  return (
    <div className="w-full bg-white min-h-screen font-sans text-slate-800 p-4 sm:p-6 md:p-8">
      <div className="max-w-7xl mx-auto py-8">
        ...
        {error && (
          <div
            className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-8 rounded-md shadow-sm"
            role="alert"
          >
          ...
          !error && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {Array.isArray(videoList) && videoList.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-300 p-8 col-span-full">
        ...
        {viewVideo && (
          <div
            className="fixed inset-0 bg-black bg-opacity-75 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-fadeIn"
                ...
                if (embed.type === "file")
                  return (
                    <video
                      ...
                      controls
                      autoPlay
                      className="w-full aspect-video rounded-lg shadow-inner bg-black"
                    />
                  );
